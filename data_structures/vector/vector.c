...
int vector_find(Vector *v, data_type val) {
    ...
    for(int i = 0; i < vector_size(v); i++) {
        if(v->compare(v->data[i], val) == 0) return i;
    }
    return find;
}
...
void vector_sort(Vector *v) {
    for(int i = 0; i < v->size; i++) {
        for(int j = i + 1; j < v->size; j++) {
            if(v->compare(v->data[i], v->data[j]) > 0) {
                vector_swap(v, i, j);
            }
        }
    }
...
int vector_binary_search(Vector *v, data_type val) {
    ...
    while(left <= right) {
        mid = (left + right)/2;
        if(v->compare(v->data[mid], val) == 0) return mid;
        else if(v->compare(v->data[mid], val) < 0) left = mid + 1;
        else right = mid - 1;
    }
    return -1;
}
