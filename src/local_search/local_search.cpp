...
void LocalSearch::run(Solution &current) {
    ...
    while (iterations_without_improvement < this->max_iterations) {
        cout << "Fitness: " << this->best_solution.fitness(this->A) << "\tIteration: " << iterations_without_improvement << endl;
        ...
        if (this->is_better_and_feasible()) {
...
void LocalSearch::get_best_neighbors(int &best_fitness, vector<Solution> &neighbors, vector<Solution> &best_neighbors) {
    // Complexidade: O(n²*m), onde n é o número de colunas e m é o número de linhas
...
bool LocalSearch::is_better_and_feasible() {
    bool is_better = this->fitness(this->best_solution) < this->fitness(this->best_legal_solution);
    bool is_feasible = this->is_feasible(this->best_solution);
    return is_better && is_feasible;
}
