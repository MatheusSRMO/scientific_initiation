...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        _results.append(("linear_programing (py)", num_sets_lp, time_lp))
        ...
        for algorithm in algorithms:
            ...
            time_c = t2 - t1
            _results.append((algorithm.replace("--", ""), num_sets_c, time_c))
    ...
    for idx, item in enumerate(results):
        ...
        for i, resultado in enumerate(resultados):
            algoritmo, execucoes, tempo = resultado
            if len(arquivo) > max_len[0]:
                max_len[0] = len(arquivo)
            if len(algoritmo) > max_len[1]:
                max_len[1] = len(algoritmo)
            if len(str(tempo)) > max_len[2]:
                max_len[2] = len(str(tempo))
    ...
    for idx, item in enumerate(results):
        ...
        for i, resultado in enumerate(resultados):
            ...
            if i == 0 and idx != 0:
                tabela.append(
                    ["-" * max_len[0], "-" * max_len[1], "-" * 8, "-" * max_len[2]]
                )
            tabela.append([arquivo, algoritmo, execucoes, tempo])

    # Exibir a tabela
    tab = tabulate(tabela, headers=['Arquivo', 'Algoritmo', 'Esquinas', 'Tempo'], tablefmt='pretty')
    print(tab)
